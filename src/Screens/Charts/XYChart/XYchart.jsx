...
export default function XYchart(props) {
  ...
  const fetchDataAndRender = async () => {
    ...
    url = `http://sustainos.ai:9000/dataservice_app/api/parameter_values/?id=${parametersId}&from_date=${fromDate}&to_date=${toDate}&data_type=line`;

    let result = await APIRequest.getGetRequestTimebasedService(url, RequestBody);
    const data = result.data.data;
    if (data) {
      console.log("parametereeee",data,parameterId)
      ...
      parametersId.forEach((key, ind) => {
        const trace = {
          x: [],
          y: [],
          name: paged === 'analytics' < 1 ? paramData.parametrList[ind].globalCode : paramData.legendType === "name" ? paramData.parameters[ind].name : paramData.parameters[ind].global,
          mode: 'lines',
          line: {
            dash: 'solid',
            width: 2.5,
            color: paged === 'analytics' < 1 ? paramData.parametrList[ind].parameterColor : paramData.parameters[ind].parameterColor
          }
        };
