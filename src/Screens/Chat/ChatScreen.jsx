                        ...
                        <>
                                    ...
                                    <View style={styles.bubblewrapperLeftStyle}>
                                        ...
                                        {item.message === 'The Weather details are listed below:' &&
                                            <View style={{ marginBottom: 20 }}>
                                                <Text style={styles.bubbleLeftTextStyle1}>
                                                ...
                                                <Text style={styles.bubbleLeftTextStyle1}>
                                                    Air Quality Co: {weatherData?.current?.air_quality?.co} μg/m3
                                                </Text>
                                                <Text style={styles.bubbleLeftTextStyle1}>
                                                    Air Quality o3: {weatherData?.current?.air_quality?.o3} μg/m3
                                                </Text>
                                                <Text style={styles.bubbleLeftTextStyle1}>
                                                    Air Quality no2: {weatherData?.current?.air_quality?.no2} μg/m3
                                                </Text>
                                                <Text style={styles.bubbleLeftTextStyle1}>
                                                    Air Quality so2: {weatherData?.current?.air_quality?.so2} μg/m3
                                                </Text>
                                            </View>}
                                    </View>
                            ...
                            {item.message === 'The Weather details are listed below:' &&
                                <ImageBackground source={weatherData?.current?.condition?.text === "Partly cloudy" ? IMAGES.Breeze : IMAGES.Sunny} style={styles.imgbg}>
                                    ...
                                    <View style={[styles.weatherwidget, { backgroundColor: weatherData?.current?.condition?.text === "Partly cloudy" ?  imageSources[1].color : imageSources[0].color }]}>
                                        <FlatList
                                            data={weatherData?.forecast?.forecastday}
                                            horizontal
                                            showsHorizontalScrollIndicator={false}
                                            renderItem={(item,index) => {
                                                const isLastItem = index === weatherData?.forecast?.forecastday?.length - 1;
                                                return (
                                                    <View style={styles.location1}>
                                                        <Text style={[styles.daydaytext,{color: COLORS.GREY}]}>{moment(item.item.date).format('ddd')}</Text>
                                                        <Text style={[styles.daydaytext]}>{item?.item?.day?.condition?.text}</Text>
                                                        {/* <Image source={{uri: item?.item?.day?.condition?.icon}} style={[styles.snow, { marginLeft: scaleWidth(3) }]} resizeMode='contain' />  */}
    ...
    const Weather = async (city) => {
        ...
        try {
            const response = await axios.get(
                `http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${cityName}&aqi=yes&days=4`
            );
        ...
        <>
            ...
            <View style={{ flex: 1, backgroundColor: '#E5E5E5' }}>
                <ImageBackground source={IMAGES.chatbg} style={{ width: '100%', height: '100%' }}>
                    <CustomHeader title={'SustainOS Hello'} navigation={navigation} icon={'menu'} />
